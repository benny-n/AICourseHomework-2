...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        alpha_beta = SearchAlgos.AlphaBeta(minimax_utility, minimax_succ, None, start_time, time_limit, minimax_heuristic)
        depth = 1
        value, direction = alpha_beta.search(curr_state, depth, 1)

        while not alpha_beta.developed_whole_tree:
            try:
                alpha_beta.developed_whole_tree = True
                value, direction = alpha_beta.search(curr_state, depth, 1)
                depth += 1
